...
/*
    The VCS contract defines three structs:

Repository: It includes the name of the repository, description, owner's address, an array of branches, and a mapping to check if a branch exists.
...
repositories: It maps the name of a repository to its information.
branches: It maps the name of a repository to its branches and each branch name to its information.
commits: It maps the name of a repository to its branches and each branch name to its commits and each commit hash to its information.

he VCS contract has two main functions for creating a repository and a branch:

createRepository: It takes the name and description of the repository as inputs and creates a new repository. It checks if the repository already exists by verifying if the "master" branch exists, and if not, it creates a new "master" branch and adds it to the repository's branches.
createBranch: It takes the name of the repository and the name of the new branch as inputs and creates a new branch. It checks if the branch already exists, and if not, it adds the new branch to the repository's branches.
